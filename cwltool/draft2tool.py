...
def revmap_file(builder, outdir, f):
    ...
    if "path" in f:
        ...
        if revmap_f:
            ...
            raise WorkflowException("Output file path %s must be within designated output directory (%s) or an input "
                                    "file pass through." % (path, builder.outdir))

    raise WorkflowException("Output File object is missing both `location` and `path` fields: %s" % f)
...
class CommandLineTool(Process):
    ...
    def job(self,
        ...
        if kwargs.get("cachedir"):
            ...
            for _, f in list(cachebuilder.pathmapper.items()):
                if f.type == "File":
        ...
        if _logger.isEnabledFor(logging.DEBUG):
            ...
            _logger.debug("[job %s] %s", j.name, json.dumps(job_order, indent=4))
        ...
        if _logger.isEnabledFor(logging.DEBUG):
            _logger.debug("[job %s] path mappings is %s", j.name,
