...
class CommandLineJob(object):
    ...
    def run(self, dry_run=False, pull_image=True, rm_container=True,
        ...
        if img_id:
            ...
            for src, vol in list(self.pathmapper.items()):
                if not vol.staged:
                    continue
                if vol.type in ("File", "Directory"):
                    runtime.append("--volume=%s:%s:ro" % (vol.resolved, vol.target))
                if vol.type == "CreateFile":
                    createtmp = os.path.join(self.stagedir, os.path.basename(vol.target))
                    with open(createtmp, "w") as f:
                        f.write(vol.resolved.encode("utf-8"))
                    runtime.append("--volume=%s:%s:ro" % (createtmp, vol.target))
            runtime.append("--volume=%s:%s:rw" % (os.path.realpath(self.outdir), self.builder.outdir))
            runtime.append("--volume=%s:%s:rw" % (os.path.realpath(self.tmpdir), "/tmp"))
            ...
            for t, v in list(self.environment.items()):
        ...
        try:
            if self.generatefiles["listing"]:
                ...
                _logger.debug("[job %s] initial work dir %s", self.name,
                ...
                def linkoutdir(src, tgt):
                    ...
                    # the container)
                    for _, item in list(self.pathmapper.items()):
        ...
        if self.stagedir and os.path.exists(self.stagedir):
            _logger.debug("[job %s] Removing input staging directory %s", self.name, self.stagedir)
