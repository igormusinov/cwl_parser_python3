...
def fillInDefaults(inputs, job):
    ...
    for e, inp in enumerate(inputs):
        with SourceLine(inputs, e, WorkflowException):
            ...
            elif shortname(inp["id"]) not in job and aslist(inp["type"])[0] == "null":
                pass
            else:
...
class Process(object, metaclass=abc.ABCMeta):
    def __init__(self, toolpath_object, **kwargs):
        ...
        if sd:
            ...
            avro.schema.SchemaFromJSONData(av, self.names)
        ...
        try:
            self.inputs_record_schema = cast(Dict[str, Any], schema_salad.schema.make_valid_avro(self.inputs_record_schema, {}, set()))
            #avro.schema.make_avsc_object(self.inputs_record_schema, self.names)
            avro.schema.SchemaFromJSONData(self.inputs_record_schema, self.names)
        except avro.schema.SchemaParseException as e:
            raise validate.ValidationException("Got error `%s` while processing inputs of %s:\n%s" %
                                               (Text(e), self.tool["id"],
                                                json.dumps(self.inputs_record_schema, indent=4)))

        try:
            self.outputs_record_schema = cast(Dict[str, Any], schema_salad.schema.make_valid_avro(self.outputs_record_schema, {}, set()))
            #avro.schema.make_avsc_object(self.outputs_record_schema, self.names)
            avro.schema.SchemaFromJSONData(self.outputs_record_schema, self.names)
        except avro.schema.SchemaParseException as e:
            raise validate.ValidationException("Got error `%s` while processing outputs of %s:\n%s" %
                                               (Text(e), self.tool["id"],
        ...
        for i, r in enumerate(hints):
            ...
            with sl:
                if avsc_names.get_name(r["class"], "") is not None:
                    ...
                    _logger.info(sl.makeError("Unknown hint %s" % (r["class"])))
