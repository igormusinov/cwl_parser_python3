...
def defaultMakeTool(toolpath_object,  # type: Dict[Text, Any]
    ...
    raise WorkflowException(
        "Missing or invalid 'class' field in %s, expecting one of: CommandLineTool, ExpressionTool, Workflow" %
        toolpath_object["id"])
...
class WorkflowJob(object):
    def __init__(self, workflow, **kwargs):
        ...
        self.name = uniquename("workflow %s" % kwargs.get("name", shortname(self.workflow.tool.get("id", "embedded"))))

        _logger.debug("[%s] initialized from %s", self.name,
    ...
    def receive_output(self, step, outputparms, jobout, processStatus):
        ...
        for i in outputparms:
            if "id" in i:
                if i["id"] in jobout:
                    ...
                    _logger.error("[%s] Output is missing expected field %s", step.name, i["id"])
        ...
        if processStatus != "success":
            ...
            _logger.warn("[%s] completed %s", step.name, processStatus)
        else:
    ...
    def job(self, joborder, output_callback, **kwargs):
            ...
            for step in self.steps:
                ...
                if not step.submitted:
                    try:
                        ...
                        _logger.error("[%s] Cannot make job: %s", step.name, e)
        ...
        try:
            ...
            _logger.error("[%s] Cannot collect workflow output: %s", self.name, e)
            wo = {}
            self.processStatus = "permanentFail"
...
class WorkflowStep(Process):
    def __init__(self, toolpath_object, pos, **kwargs):
        ...
        if "scatter" in self.tool:
            ...
            for s in scatter:
                if s not in inp_map:
                    raise WorkflowException("Scatter parameter '%s' does not correspond to an input parameter of this "
                                            "step, inputs are %s" % (s, list(inp_map.keys())))
    ...
    while rc.completed < rc.total:
        ...
        for index in range(len(steps)):
            ...
            try:
                ...
                _logger.error("Cannot make scatter job: %s", e)
                _logger.debug("", exc_info=True)
