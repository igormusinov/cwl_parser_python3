...
from .process import normalizeFilesDirs, use_custom_schema, use_standard_schema
#from .process import (shortname, Process, relocateOutputs, cleanIntermediate, scandeps, normalizeFilesDirs, use_custom_schema, use_standard_schema)
...
def load_job_order(args, t, stdin, print_input_deps=False, relative_deps=False,
    ...
    if job_order_object:
        ...
        if toolparser:
            ...
            job_order_object.update({namemap[k]: v for k, v in list(cmd_line.items())})
...
def main(argsl=None,  # type: List[str]
    ...
    try:
        ...
        for k, v in list({'print_deps': False,
                     'print_pre': False,
                     'print_rdf': False,
                     'print_dot': False,
                     'relative_deps': False,
        ...
        try:
            ...
            if status != "success":
                _logger.warn("Final process status is %s", status)
                return 1
            else:
            ...
            _logger.error(
                "Workflow error, try again with --debug for more "
                "information:\n%s", strip_dup_lineno(str(exc)), exc_info=args.debug)
            return 1
        except Exception as exc:
            _logger.error(
